...
def init(default):
    ...
    if default:
        ...
        if not os.path.exists("dataset.yaml"):
            print("[bold]Creating a dataset configuration file...[/bold]")
            ...
            if f:
                ...
                print(
                    "Dataset YAML file has been created sucessfully. Now run [bold blue]idt build[/bold blue] to mount your dataset!")
                exit(0)

        else:
            print(
                "[red]A dataset.yaml is already created. To use another one, delete the current dataset.yaml file[/red]")
            exit(0)
    ...
    print("[bold]Choose image resolution[/bold]", justify="center")
    print("""
...
[italic]ps: note that the aspect ratio of the image will [bold]not[/bold] be changed, so possibly the images received will have slightly different size[/italic]
    ...
    while image_size_ratio < 1 or image_size_ratio > 5:
        print("[italic red]Invalid option, please choose between 1 and 5. [/italic red]")
    ...
    print("[bold]Choose a resize method[/bold]", justify="center")
    print("""
...
[italic]ps: note that the aspect ratio of the image will [bold]not[/bold] be changed, so possibly the images received will have slightly different size[/italic]
    ...
    print("[bold]Choose a search engine[/bold]", justify="center")
    print("""
...
[4] Flickr API [italic yellow](Requires API key)[/italic yellow]
    ...
    while search_engine < 0 or search_engine > 4:
        print("[italic red]Invalid option, please choose between 1 and 4.[/italic red]")
    ...
    if search_engine > 2:
        ...
        print(f'Insert your [bold blue]{search_options[search_engine]}[/bold blue] API key')
    ...
    for x in range(number_of_classes):
        ...
        print("""In order to achieve better results, choose several keywords that will be provided to the search engine to find your class in different settings.
...
[italic]keywords[/italic]: [underline]pineapple, pineapple fruit, ananas, abacaxi, pineapple drawing[/underline]
    ...
    if not os.path.exists("dataset.yaml"):
        ...
        try:
            ...
            print("[red]Unable to create file. Please check permission[/red]")

    else:
        print("[red]A dataset.yaml is already created. To use another one, delete the current dataset.yaml file[/red]")
...
def build():
    ...
    for classes in data['CLASSES']:
        ...
        print('Creating [bold blue]{name} class[/bold blue] \n'.format(name=classes['CLASS_NAME']))
...
def split():
    ...
    while True:
        ...
        print(3, BANNER)
        print(
            "Choose the desired proportion of images of each class to be distributed in train/valid folders. [bold]What percentage of images should be distributed towards training?[/bold] ")
        ...
        if train_proportion < 0 or train_proportion > 100:
            ...
            print(
                "[red]Please provide a valid amount. Choose a number between 0 and 100 to be assigned to training.[/red]")
            continue
        else:

            print("[bold blue]{train} percent[/bold blue] of the images will be moved to a [bold yellow]train[/bold yellow] folder, while [bold blue]{valid} percent [/bold blue] of the remaining images will be stored in a [bold yellow]validation[/bold yellow] folder.".format(
                train=train_proportion, valid=validation_proportion))
            ...
            if c.lower() == 'y':
                ...
                with open('dataset.yaml') as f:
                    ...
                    print("[italic]Copying files to the train/valid folders. Please wait...[/italic]")
                    data = yaml.load(f, Loader=yaml.FullLoader)
